...
0. context.Context 必须显式传递，不存全局 context
1. 通道使用带缓冲或无缓冲要明确意图
2. 接口设计要小，优先组合
3. 使用 defer 释放资源，注意 defer 内的错误处理
4. 切片传引用，修改前确认是否会影响调用方

## 常见陷阱

0. 在循环中使用 goroutine 闭包捕获变量
1. nil map 直接写入会 panic
2. 切片 append 后底层数组共享导致意外修改
3. 忘记检查 error 返回值
4. 使用 default 的 http.Client 没有超时
